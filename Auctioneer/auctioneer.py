...
class Auctioneer(object):
	...
	def __init__(self, 
			...
			target_collection=[3,3,1,1], # How many types of paintings needed in collection game. [3,2,1] means 3 of one, 2 of another and 1 of another
			slowdown=0.1, # Time to wait between rounds. Set to 0 for fast auctions
	...
	def __initialise_bots(self):
		...
		count = 0
		for bot_class in self.room:
			if self.verbose:
				print("Initialising {}".format(bot_class))
			count += 1
			...
			try:
				...
				self.__log_error(bot_name=bot_class, game_stage="initialisation", error_message=str(e))
				raise
			...
			try:
				...
				print("Error caught ", str(e))
				self.__log_error(bot_name=bot_class, game_stage="initialisation - bot name", error_message=str(e))
				raise
